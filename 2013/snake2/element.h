...
class Text:public Rect//文字输出类
{
	...
	DWORD color;
public:
	Text(int rx,int ry,int rw,int rh,const char* face="宋体");
	~Text(){if(font)DeleteObject(font);}
	void SetText(const string& tex,DWORD texcolor=RGB(0,0,0)){str=tex;color=texcolor;};
	void SetText(int num,DWORD texcolor=RGB(0,0,0)){ostringstream ss;ss<<num;str=ss.str();color=texcolor;}
	void Draw();
};
...
public:
	static void PreInit(HWND hWnd){hwnd=hWnd;}
